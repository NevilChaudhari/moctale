...
export default function loginCard() {
    ...
    const allFilled = (usernameLogin ? username.trim() !== '' : phoneNo.trim() !== '') && password.trim() !== '';
    ...
    return (
        <div className="bg-white rounded-lg flex flex-col justify-center items-center w-[48%] h-auto p-5 md:max-w-90">
            ...
            <div className="bg-gray-100 rounded-lg w-full p-1 flex mb-4">
                <button className={`cursor-pointer border-0 rounded-md flex-1 py-2 text-sm font-medium ${usernameLogin ? "bg-white text-black shadow-sm" : "text-gray-500"}`} onClick={() => {setUsernameLogin(true); setPhoneLogin(false)}}>
                    Username
                </button>
                <button className={`cursor-pointer bg-none border-0 rounded-md flex-1 py-2 text-sm font-medium ${!usernameLogin ? "bg-white text-black shadow-sm" : "text-gray-500"}`} onClick={() => {setUsernameLogin(false); setPhoneLogin(true)}}>
                    Phone
                </button>
            </div>

            {/* Username */}
            {usernameLogin && (<label className='text-black self-start text-sm mb-1 font-semibold'>Username</label>)}
            {usernameLogin && (<input type='text' onChange={(e) => setUsername(e.target.value)} value={username} className='mb-2.5 border border-gray-300 w-full px-2 rounded-md h-10 text-black placeholder:text-gray-400 focus:outline-0 focus:ring-1 focus:ring-black/10' placeholder='Enter Your Username'></input>)}

            {/* Phone */}
            {phoneLogin && (<label className='text-black self-start text-sm mb-1 font-semibold'>Phone Number</label>)}
            {phoneLogin && (<div className='mb-2.5 w-full flex items-center text-black'>
